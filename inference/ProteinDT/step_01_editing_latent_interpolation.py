...
from utils import slerp, ProteinDatasetMask, ProteinDatasetNoMask,ProteinDataset, ProteinSeqDataset, text_prompt_dict, load_oracle_evaluator, evaluate, analyze
...
def inference(dataloader, theta, text_condition_repr, device):
    ...
    for batch_idx, batch in enumerate(L):
        ...
        if args.decoder_distribution in ["T5Decoder"] and args.evolution in ["mutation"]:
            protein_sequence_input_ids_t5 = batch["protein_sequence_input_ids_t5"].to(device)
        # Replace BERT's [MASK] with T5's <extra_id_0>
       # t5_compatible_sequences = [seq.replace("[MASK]", "<extra_id_0>") for seq in protein_sequence_batch]

        # Tokenize the sequences for T5 model
        #t5_input_ids = protein_decoder_tokenizer(t5_compatible_sequences, return_tensors="pt", padding=True, truncation=True).input_ids.to(device)

        #print(f"seq input \n {protein_sequence_batch}")
        #print(f"seq encoded\n {t5_input_ids}")


        protein_masked_positions = batch["protein_masked_positions"]
        protein_sequence_input_ids = batch["protein_sequence_input_ids"].to(device)
        print("outside protein_sequence_input_ids")
        print(protein_sequence_input_ids.shape)
        protein_sequence_attention_mask = batch["protein_sequence_attention_mask"].to(device)
        B = len(protein_sequence_batch)
        
        #unmasking
        unmasking_attention_mask = torch.zeros_like(protein_sequence_input_ids)
        for i, mask_pos in enumerate(protein_masked_positions):
            unmasking_attention_mask[i, mask_pos] = 1

        #print("size outside")
        #print(unmasking_attention_mask.shape)
        ...
        if args.AR_condition_mode == "aggregated":         
            ...
            #print(repeated_condition_repr.shape)
        ...
        if args.decoder_distribution in ["T5Decoder"] and args.evolution in ["mutation"]:
            ...
            # )
            #print("generation condition")
            #print(unmasking_attention_mask)
            protein_sequence_pred_ids = decoder_distribution_model.mutate(
            condition=repeated_condition_repr,
            #this is bert id converted to t5 id
            original_ids=protein_sequence_input_ids_t5,
            masked_positions=protein_masked_positions,
            max_seq_len=max_len,
        ...
        #     protein_sequence_pred_ids = decoder_distribution_model.mutate(
        #     condition=repeated_condition_repr,
        #     protein_seq_input_ids=protein_sequence_input_ids,
        #     max_seq_len=max_len,
        #     protein_seq_attention_mask=protein_sequence_attention_mask,
        #     masked_positions=protein_masked_positions,  # Ensure this is passed and properly formatted
        #     temperature=temperature,
        #     k=k,
        #     p=p,
        #     repetition_penalty=repetition_penalty,
        ...
        # )
        ...
        elif args.decoder_distribution in ["T5Decoder"] and args.evolution in ["EC"]:
            max_len = max(protein_sequence_attention_mask.sum(1)).item()
            if args.AR_generation_mode == "01":
                temperature = 1.0
                k = 40
            ...
            #     condition=repeated_condition_repr, protein_seq_attention_mask=protein_seq_attention_mask, max_seq_len=args.protein_max_sequence_len)

            protein_sequence_output = decoder_distribution_model.mutate(
                protein_masked_positions=protein_masked_positions,
        ...
        #print('the mask pos is ')
        #print(protein_masked_positions)
        #print("we about to decode")
        #print(protein_sequence_pred_ids)
        ...
        #print("results is")
        print(candidate_protein_sequence_list)
    ...
    print(input_protein_sequence_list)
...
@torch.no_grad()
def inference_EC(dataloader, theta, text_condition_repr, device):
    if args.verbose:
        L = tqdm(dataloader)
    else:
        L = dataloader
...
if __name__ == "__main__":
    ...
    text_prompt = "modify the amino acid sequence so that it removes a phosphate group from [pyruvate, phosphate dikinase] phosphate and that it is a evolutionary homolog of enzymes that removes a phosphate group from [pyruvate, water dikinase] phosphate."
    ...
    #dataset_file_path = "/home/tonyni/ESM3/protein-gym/Artificial_Mutation/part_1_artificial_double_CATR_CHLRE_Tsuboyama_2023_2AMI.csv"
    ...
    dataset_file_path = "/home/tonyni/pdt/ProteinDT/examples/downstream_Editing/enzymes.csv"
    ...
    elif args.evolution in ["EC"]:
        input_protein_sequence_list, edited_protein_sequence_list = inference_EC(dataloader, args.theta, text_prompt_condition_repr, device)
