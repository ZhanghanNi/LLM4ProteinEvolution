...
class MultinomialDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def mutate(self, protein_masked_positions,protein_sequence_input_ids,condition, max_seq_len, protein_seq_attention_mask):
        """
        ...
        x_t = protein_sequence_input_ids.clone()
        unmasked = torch.ones_like(x_t, device=device).bool()
        for i in range(B):
            x_t[i, protein_masked_positions[i]] = self.mask_id
            unmasked[i, protein_masked_positions[i]] = False  # Set these positions as masked (need to be updated)

        temperature = 1.

        for timestep in reversed(range(1, 1+self.num_diffusion_timesteps)):
            t = torch.full((B,), timestep, device=device).long()  # (B)
            changes = torch.zeros_like(x_t, device=device).bool()
            for i in range(B):
            # Only the positions that are originally masked can change
                changes[i, protein_masked_positions[i]] = True
    ...
    def simplified_inference(self, condition, max_seq_len, protein_seq_attention_mask):
        ...
        condition = condition.float()
        condition = self.condition_proj_layer(condition)  # (B, max_seq_len, condition_dim) ---> (B, max_seq_len, hidden_dim)
        print(f"Cond is {condition.shape}")


        x_t = torch.ones((B, max_seq_len), device=device).long() * self.mask_id
        print(f"xt is {x_t.shape}")
        unmasked = torch.zeros_like(x_t, device=device).bool()
        print(f"unmasked is {unmasked.shape}")
        temperature = 1.
