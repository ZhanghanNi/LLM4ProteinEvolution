...
from utils import ProteinDataset,ProteinDatasetNoMask,ProteinDatasetMask, ProteinSeqDataset, text_prompt_dict, load_oracle_evaluator, evaluate, analyze
...
def inference(dataloader, text_prompt_CLAP_repr, device):
    ...
    for batch_idx, batch in enumerate(L):
        ...
        for sequence in edited_protein_sequence_list:
            print(f"Length of sequence: {len(sequence)}")

    return input_protein_sequence_list, edited_protein_sequence_list

def mask_protein_sequence(protein_sequence, mask_indices):
    # Ensure the indices are within the valid range
    for index in mask_indices:
        if not (0 <= index < len(protein_sequence)):
            raise ValueError(f"mask_index {index} is out of range")
    
    # Insert spaces between the characters of the protein sequence
    spaced_sequence = " ".join(protein_sequence)
    
    # Split the spaced sequence into a list of characters
    spaced_list = spaced_sequence.split()
    
    # Replace the characters at the specified indices with "[MASK]"
    for index in mask_indices:
        spaced_list[index] = "[MASK]"
    
    # Join the list back into a string
    masked_sequence = " ".join(spaced_list)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text_prompt_id", type=int, default=201)
    ...
    parser.add_argument("--evolution", type = str, default = "mutation")
    ...
    #text_prompt = "modify the amino acid sequence so that it removes a phosphate group from [pyruvate, phosphate dikinase]-phosphate."
    #text_prompt = "Modify this amino acid sequence to catalyze the conversion of sorbitol to fructose."
    #text_prompt = "Modify this amino acid sequence to function as NAD-dependent sorbitol dehydrogenase, catalyzing the conversion of L-iditol to L-sorbose"

    #text_prompt = 'Modify this amino acid sequence to evolve into its homolog that catalyzes the conversion of sorbitol to fructose.'
    #text_prompt = "Modify this amino acid sequence to evolve into its homolog that functions as NAD-dependent sorbitol dehydrogenase, catalyzing the conversion of L-iditol to L-sorbose."

    #text_prompt = "Modify this amino acid sequence to catalyze the conversion of sorbitol to fructose, resulting in an evolutionary homolog"
    #text_prompt = "Modify this amino acid sequence to function as NAD-dependent sorbitol dehydrogenase to catalyze the conversion of L-iditol to L-sorbose, resulting in an evolutionary homolog"

    #text_prompt= "Modify this amino acid sequence to enhance its activity on L-iditol as a substrate, converting it into L-sorbose with NAD+ as the oxidizing agent, reflecting the enzymatic function of sorbitol dehydrogenase."

    #text_prompt = "sorbitol dehydrogenase"
    #text_prompt = "polyol dehydrogenase"

    #text_prompt = "Modify this amino acid sequence to become polyol dehydrogenase and similar to L-iditol 2-dehydrogenase, D-xylulose reductase, butyleneglycol dehydrogenase, and L-xylose dehydrogenase."
    
    #14
    #text_prompt = "Modify this amino acid sequence to become that of sorbitol dehydrogenase."
    #text_prompt = "Modify this amino acid sequence to become that of sorbitol dehydrogenase, which also is an enzyme classified as an oxidoreductase, acting on the CH-OH group of donors, with NAD⁺ or NADP⁺ as acceptor."
    #text_prompt = "Modify this amino acid sequence to become that of sorbitol dehydrogenase, which catalyzes the conversion of L-iditol to L-sorbose."
    #text_prompt = "Modify this amino acid sequence to become that of sorbitol dehydrogenase. The following enzymes become increasingly similar to sorbitol dehydrogenase: aryl-alcohol dehydrogenase, lactaldehyde reductase, isopropanol dehydrogenase, L-threonine 3-dehydrogenase, and glucose 1-dehydrogenase."

    #55
    #text_prompt="Modify this amino acid sequence to become that of lactaldehyde reductase."
    #text_prompt="Modify this amino acid sequence to become that of lactaldehyde reductase, which also is an enzyme classified as an oxidoreductase, acting on the CH-OH group of donors, with NAD⁺ or NADP⁺ as acceptor."
    #text_prompt="Modify this amino acid sequence to become that of lactaldehyde reductase, which catalyzes the conversion of propane-1,2-diol to L-lactaldehyde."
    text_prompt="Modify this amino acid sequence to become that of lactaldehyde reductase, considering that aryl-alcohol dehydrogenase is more similar to lactaldehyde reductase than the original sequence."

    #36

    #text_prompt="Modify this amino acid sequence to become that of acetoacetyl coenzyme A reductase."
    #text_prompt="Modify this amino acid sequence to become that of acetoacetyl coenzyme A reductase, which also is an enzyme classified as an oxidoreductase, acting on the CH-OH group of donors, with NAD⁺ or NADP⁺ as acceptor."
    #text_prompt="Modify this amino acid sequence to become that of acetoacetyl coenzyme A reductase, which catalyzes the conversion of (R)-3-hydroxyacyl-CoA to 3-oxoacyl-CoA."
    #text_prompt="Modify this amino acid sequence to that of acetoacetyl coenzyme A reductase. The following enzymes become increasingly similar to acetoacetyl coenzyme A reductase: aryl-alcohol dehydrogenase, lactaldehyde reductase, isopropanol dehydrogenase, L-threonine 3-dehydrogenase, D-iditol 2-dehydrogenase, 2-hydroxy-3-oxopropionate reductase, 3-hydroxypropionate dehydrogenase, quinate dehydrogenase, aromatic 2-oxoacid reductase, ketol-acid reductoisomerase, malate dehydrogenase, lactaldehyde reductase, propanediol:NAD oxidoreductase, L-xylose 1-dehydrogenase, alcohol dehydrogenase, 3-dehydrosphinganine reductase, and glucose 1-dehydrogenase."
    ...
    #dataset_file_path = "/home/tonyni/ESM3/protein-gym/DMS_substitution_positions/CATR_CHLRE_Tsuboyama_2023_2AMI.csv"
    #finetune_dataset_file_path = "/home/tonyni/pdt/ProteinDT/examples/downstream_Editing/test.csv"
    finetune_dataset_file_path = "/home/tonyni/pdt/ProteinDT/examples/downstream_Editing/enzymes1.1.1.36.csv"
    #dataset_file_path = "/home/tonyni/ESM3/protein-gym/Artificial_Mutation/artificial_single_CATR_CHLRE_Tsuboyama_2023_2AMI.csv"
    #dataset_file_path = "/home/tonyni/pdt/ProteinDT/examples/downstream_Editing/test.csv"
    #dataset_file_path = "/home/tonyni/ESM3/protein-gym/Artificial_Mutation/shuffled/shuffled_part_1_artificial_double_CATR_CHLRE_Tsuboyama_2023_2AMI.csv"
    dataset_file_path = "/home/tonyni/pdt/ProteinDT/examples/downstream_Editing/enzymes1.1.1.1.csv"

    if args.evolution in ["EC"]:
        finetune_dataset = ProteinDataset(
        dataset_file_path=finetune_dataset_file_path,
        dataset_size=args.dataset_size,
        protein_tokenizer=CLAP_protein_tokenizer,
        protein_max_sequence_len=args.protein_max_sequence_len)
        finetune_dataloader = DataLoader(finetune_dataset, batch_size=16, shuffle=True, num_workers=args.num_workers)
    ...
    #print(dataset_file_path)
        finetune_dataset = ProteinDatasetNoMask(
        dataset_file_path=finetune_dataset_file_path,
        dataset_size=args.dataset_size,
        protein_tokenizer=CLAP_protein_tokenizer,
        protein_max_sequence_len=args.protein_max_sequence_len)
    ...
    for _ in range(args.AE_epochs):
         finetune_AE(finetune_dataloader, CLAP_protein_model, CLAP_protein2latent_model, CLAP_protein_tokenizer, optimizer, CE_criterion)
    ...
    print(edited_protein_sequence_list)
